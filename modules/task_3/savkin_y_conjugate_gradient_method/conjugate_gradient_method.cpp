...
std::vector<double> conjugateGradientMethodSave(double** a, const double* b, int size) {
    int comm_size, comm_rank;
    MPI_Comm_size(MPI_COMM_WORLD, &comm_size);
    MPI_Comm_rank(MPI_COMM_WORLD, &comm_rank);
    ...
    std::vector<double> x, w_all, r, w(rows), z_all(size);
    ...
    for (int i = 0; i < size; ++i) {
        mat_vec_mul(m, z_all, &w);
        if (comm_rank == 0) {
            MPI_Gatherv(w.data(), rows, MPI_DOUBLE, w_all.data(), counts.data(), displs.data(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
        } else {
            MPI_Gatherv(w.data(), rows, MPI_DOUBLE, nullptr, nullptr, nullptr, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        }
        if (comm_rank == 0) {
            double h = vector_mul(w_all, z_all);
            ...
            double g = vector_mul(r, r);

            double t2 = g / u;
            u = g;
            for (int j = 0; j < size; ++j) {
                z_all[j] = t2 * z_all[j] + r[j];
            }
        }
